...
from mhg.corpus import CliqueTreeCorpus
from mhg.base import GraphGrammarBase
from mhg.symbols import TSymbol, NTSymbol, BondSymbol
from mhg.utils import _node_match, _node_match_prod_rule, _edge_match, masked_softmax, common_node_list
from mhg.hypergraph import Hypergraph
from collections import Counter
from copy import deepcopy
from mhg.tree_decomposition import (
    tree_decomposition,
    ...
    def learn(self, hg_list, logger=print, max_mol=np.inf, print_freq=500):
        ...
        for each_idx, each_hg in enumerate(hg_list):
            ...
            if each_idx % 10 == 0:
                ...
                if each_idx % 10 == 0 and each_idx != 0:
                    each_idx = str(each_idx)
                    save_point.append(each_idx)
                    with open('OUTPUT/extracted_rules.txt', mode='w') as f:
                        f.write('\n'.join(save_point))
                        #print("save")
            
            each_idx = int(each_idx)
