...
loss_catalog = {
    ...
    'GrammarVAELoss_beta': GrammarVAELoss_beta,
    'VAELoss': VAELoss}
...
class Seq2SeqBase(nn.Module):
    ...
    def fit(self, data_loader_train, data_loader_val=None, max_num_examples=None,
            print_freq=1000, num_epochs=10, sgd=Adagrad, sgd_kwargs={}, logger=print, save_path = None):
        ''' fit to the data
        Parameters
        ----------
        ...
        for epoch in range(num_epochs):
            if not finish:
                for each_idx, each_batch in enumerate(data_loader_train):
                    ...
                    print("epoch:",epoch + 1,"/",num_epochs,"batch:",each_idx + 1,"/",len(data_loader_train))
                    
                    log = "epoch:" + str(epoch + 1) + "/" + str(num_epochs) + "batch:" + str(each_idx + 1) + "/" + str(len(data_loader_train))
                    ...
                    if i % print_freq == print_freq-1:
                        ...
                        print(msg)
                        self.train_loss_log.append(msg)
                        with open(os.path.join(save_path, 'train_loss_log.txt'), mode='w') as f:
                            f.write('\n'.join(self.train_loss_log))
                            print("save train loss")
                            
                        self.train_log.append(log)
                        with open(os.path.join(save_path, 'train_log.txt'), mode='w') as f:
                            f.write('\n'.join(self.train_log))
                        print("save log")
                ...
                if data_loader_val is not None:
                    ...
                    msg = f'epoch: {epoch + 1}\t '\
                          f'per-example val_loss: {running_val_loss / (self.batch_size * (each_idx + 1))}'
                    
                    print(msg)
                    self.val_loss_log.append(msg)
                    with open(os.path.join(save_path, 'val_loss_log.txt'), mode='w') as f:
                        f.write('\n'.join(self.val_loss_log))
                        print("save val loss")
    ...
    def val(self, data_loader_train, data_loader_val=None, max_num_examples=None,
            print_freq=1000, num_epochs=10, sgd=Adagrad, sgd_kwargs={}, logger=print, save_path = None):
        ''' fit to the data
        Parameters
        ----------
...
class Seq2SeqVAE(Seq2SeqBase):
    ...
    #loss.py内に記述されてるGrammarVAELossが
    #criterionに対応するので、loss.pyを書き換えれば、
    #returnされる値も変わる
