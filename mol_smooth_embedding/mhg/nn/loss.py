...
class GrammarVAELoss_beta(_Loss):
    ...
    def __init__(self, hrg, ignore_index=None, beta=0.01, class_weight=None, **kwargs):
        super().__init__(**kwargs)
        self.hrg = hrg
        self.ignore_index = ignore_index
        self.beta = beta
    ...
    def forward(self, in_seq_pred, in_seq, mu, logvar, beta):
        ''' compute VAE loss
        ...
        cross_entropy = F.cross_entropy(
            ...
            ignore_index=self.ignore_index if self.ignore_index is not None else -100)
        kl_div = -0.5 * torch.sum(1 + logvar - mu.pow(2) - logvar.exp())
        return cross_entropy + self.beta * kl_div, cross_entropy, kl_div