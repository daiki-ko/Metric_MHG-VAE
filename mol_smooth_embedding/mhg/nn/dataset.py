...
class HRGDataset2(Dataset):

    '''
    A class of HRG data
    '''

    def __init__(self, hrg, prod_rule_seq_list, max_len, target_val_list1=None, target_val_list2=None,  inversed_input=False):
        self.hrg = hrg
        self.left_prod_rule_seq_list = left_padding(prod_rule_seq_list,
                                                    max_len,
                                                    inverse=inversed_input)
        ...
        self.right_prod_rule_seq_list = right_padding(prod_rule_seq_list, max_len)
        self.inserved_input = inversed_input
        self.target_val_list1 = target_val_list1
        self.target_val_list2 = target_val_list2
        
    def __len__(self):
        return len(self.left_prod_rule_seq_list)

    def __getitem__(self, idx):
        if self.target_val_list2 is not None and self.target_val_list1 is not None:
            return self.left_prod_rule_seq_list[idx], self.right_prod_rule_seq_list[idx], np.float32(self.target_val_list1[idx]) \
                        ,np.float32(self.target_val_list2[idx])
